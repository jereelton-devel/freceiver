...
function getPecas() {
    ...
    $.ajax({
        ...
        success: function ($data) {
            ...
            $.each($json, function ($i, $val) {
                $("#produto_id_peca").append('<option value="' + $val.produto_id + '">' + $val.produto_codigo + ' - ' + $val.produto_titulo + ' - ' + $val.produto_localizacao + '</option>');
            });
        }
    });
}
...
function __listaAgAnalise($json) {
    ...
    $.each($json, function ($i, $val) {
        ...
        //var $inputQtdDefeito = '<div class="input-control"><input disabled type="number" name="osprodutopeca_defeito[]" step="1" min="0" max="'+$val.osprodutopeca_qtde+'" value="'+$val.qtde_defeito+'" /></div>';
        ...
        var $tr = '<tr><td>' + $index + '</td><td>DI0003</td><td><input type="text" class="produto_diagnostico_' + $val.produto_id_peca + '" onkeyup="diagnosticoCode(' + $val.produto_id_peca + ')" value="' + $val.diagnostico_code + '"></td><td>' + $peca + '</td><td>' + $val.produto_localizacao + '</td><td><input type="text" class="produto_resolucao_' + $val.produto_id_peca + '" onkeyup="resolucaoCode(' + $val.produto_id_peca + ')" value="' + $val.resolucao_code + '"></td><td>' + $inputQtd + '</td><td>' + $qtdEstoque + '</td><td>' + $upd + $dlt + $laudo + '</td></tr>';
        $html.push($tr);
    });
...
function __listaAgPeca($json) {
    ...
    $.each($json, function ($i, $val) {
        ...
        var $tr = '<tr><td>' + $index + '</td><td>' + $falha + '</td><td>' + $solucao + '</td><td>' + $peca + '</td><td>' + $inputQtd + '</td><td>' + $inputQtdReuso + '</td><td>' + $inputQtdDefeito + '</td><td>' + $qtdEstoque + '</td><td>' + $estoque + '</td><td>' + $dlt + $troca + '</td></tr>';
        $html.push($tr);
    });
...
function __listaReparo($json) {
    ...
    $.each($json, function ($i, $val) {
        ...
        var $inputQtdDefeito = '<div class="input-control"><input type="number" class="qtde_defeito" name="osprodutopeca_defeito[]" step="1" min="0" max="' + $val.osprodutopeca_qtde + '" value="' + $val.qtde_defeito + '" /></div>';
        var $QtdDefeito = '<div class="input-control"><input type="hidden" name="qtde_defeito[]" value="' + $val.qtde_defeito + '" /></div>';
        var $laudo = '<a href="javascript: void(0);" onclick="dialogGeraLaudoSolucoes(' + $i + ');" title="Imprimir laudo" alt="Imprimir laudo"><i class="icon-printer"></i></a>';

        var $tr = '<tr><td>' + $index + '</td><td>' + $falha + '</td><td>' + $solucao + '</td><td>' + $peca + '</td><td>' + $inputQtd + '</td><td>' + $inputQtdReuso + '</td><td>' + $inputQtdUti + '</td><td>' + $inputQtdDefeito + $QtdDefeito + '</td><td>' + $qtdEstoque + '</td><td>' + $estoque + '</td><td>' + $laudo + '</td></tr>';
        $html.push($tr);
    });
...
function addListaSolucoes() {
    ...
    var $servico_id = parseInt($("#servico_id").val()) || 1;
    var $defeito_id = parseInt($("#defeito_id").val()) || 101;
...
function dltListaSolucoes($index, $exchange) {
    ...
    $.Dialog({
        ...
        'buttons': {
            'OK': {
                'action': function () {
                    var defeito_confirmar = $("#defeito_confirmar").val();
                    // alert(defeito_confirmar);
                    // return;
...
function getDelete($index) {
    ...
    $.ajax({
        type: "POST",
        url: 'os_estoque.php',
        data: { acao: 'get_defeito_confirmar' },
        async: false,
        success: function (data) {
            dialog_filtro = data;
        }
